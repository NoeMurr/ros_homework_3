...
{
    std::puts("Saving PCD file");

    auto mapPoints = mpMap->GetAllMapPoints();
    auto width = mapPoints.size();
    ...
    for(auto point : mapPoints){
        ...
        auto mpPos = point->GetWorldPos();
        outputFile << setprecision(7)<< mpPos.at<float>(0) << " " << mpPos.at<float>(1) << " " << mpPos.at<float>(2) << endl;
    } 

    outputFile.close();
    std::puts("pcd file saved");
